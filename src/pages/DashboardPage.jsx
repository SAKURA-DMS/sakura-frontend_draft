              ...
              {activity.map((act) => (
                <div key={act.id} className="flex items-start gap-3">
                  {act.userAvatar ? (
                    ...
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 mt-0.5 text-primary text-xs font-bold">
                      {(act.userName || "?")[0]}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
...
function PersetujuanTab({ currentUser, canApprove, approveDocument, rejectDocument, onSelectDoc, onDocUpdated }) {
  ...
  const handleApprove = async (req) => {
    await approveDocument(req.document_id, approveComment.trim() || undefined);
    setApproveId(null);
    setApproveComment("");
    fetchPending();
    onDocUpdated?.();
  ...
  const handleReject = async (req) => {
    ...
    await rejectDocument(req.document_id, rejectReason.trim());
    setRejectId(null);
    setRejectReason("");
    fetchPending();
    onDocUpdated?.();
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-muted/20">
                <div className="flex items-center gap-2.5">
                  {req.requester_avatar ? (
                    <img src={req.requester_avatar} alt="" className="w-8 h-8 rounded-lg object-cover" />
                  ) : (
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary text-xs font-bold">
                      {(req.requester_nama || "?")[0]}
                    </div>
                  )}
                  <div>
