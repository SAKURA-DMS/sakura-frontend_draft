...
function AvatarImage({ src, nama, className }) {
  ...
  // Reset saat src berubah (misal setelah user upload avatar baru)
  useEffect(() => {
    setBroken(false);
  }, [src]);

  // Ambil inisial (maks 2 huruf) dari nama
  ...
  // 1. src tidak ada / null / undefined
  // 2. src tidak dimulai dengan "data:" atau "http" (pasti invalid)
  // 3. onError dipanggil browser (image gagal load)
  const isValidSrc =
    src &&
    !broken &&
